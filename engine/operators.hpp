...
struct TransformerComputing {
    ...
    virtual std::variant<ComputingReturn, tensor_t> op_clone(tensor_t self, const std::vector<size_t>& newShape, const char* dtype) {
        return OP_TODO_ERROR;
    }
    ...
    virtual ComputingReturn op_sampling_top_p(tensor_t self, tensor_t mask, tensor_t ids, float temp, float top_p) {
        return OP_TODO_ERROR;
    }
